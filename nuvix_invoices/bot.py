...
@bot.tree.command(name="invoice_create", description="Create a new invoice")
...
async def invoice_create(inter: discord.Interaction, user: discord.User, amount: float, note: str):
    ...
    await inter.response.send_message(embed=emb("Invoice created", f"For: {user.mention}\nAmount: **{amount:.2f}**\nNote: {note}"))

@bot.tree.command(name="invoice_status", description="Check an invoice status")
async def invoice_status(inter: discord.Interaction, invoice_id: str):
    await log_cmd(inter, "invoice_status")
    await inter.response.send_message(embed=emb("Invoice status", f"Invoice `{invoice_id}` is **Pending**"))
...
bot.run(os.getenv("NUVIX_INVOICES_TOKEN"))
